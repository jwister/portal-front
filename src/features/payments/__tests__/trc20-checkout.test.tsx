...
describe('Trc20Checkout', () => {
  ...
  it('renders only server-issued payment instructions and submits the TxID to its order endpoint', async () => {
    ...
    expect(await screen.findByText('25.501 USDT')).toBeVisible()
    ...
    expect(screen.getByTestId('trc20-ledger-summary')).toHaveTextContent('Recharge amount: $25.50')
    expect(screen.getByTestId('trc20-ledger-summary')).not.toHaveTextContent('Expected credit')
