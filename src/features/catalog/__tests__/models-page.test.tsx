...
describe('ModelsPage', () => {
  ...
  it('filters model cards by model name', async () => {
    ...
    await screen.findByRole('link', { name: 'gpt-5-mini' })
    ...
    expect(screen.getByRole('link', { name: 'glm-5' })).toBeVisible()
    expect(screen.queryByRole('link', { name: 'gpt-5-mini' })).not.toBeInTheDocument()
  })

  it('exposes the refreshed catalog hierarchy and card sections', async () => {
    render(<ModelsPage />)

    expect(await screen.findByRole('link', { name: 'gpt-5-mini' })).toBeVisible()
  ...
  it('filters models by selected group and keeps search scoped to that group', async () => {
    ...
    expect(screen.getByRole('link', { name: 'gpt-5-mini' })).toBeVisible()
    expect(screen.queryByRole('link', { name: 'glm-5' })).not.toBeInTheDocument()
